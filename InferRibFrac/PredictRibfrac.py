...
class Config():
    ...
    def __init__(self):
        self.image_dir = "./"
        self.pred_dir = "./PredictDir/"
        self.model_path = "model_weights.pth"
...
batch_size = 16
num_workers = 4
postprocess = True if config.postprocess == "True" else False

image_path = "/root/workspace/RibFrac/ribfrac-test-images/RibFrac501-image.nii.gz"
image_id = "RibFrac501"
...
pred_arr = _predict_single_image(model, dataloader, postprocess,
    config.prob_thresh, config.bone_thresh, config.size_thresh)
pred_image, pred_info = _make_submission_files(pred_arr, image_id,dataset.image_affine)
pred_path = os.path.join(config.pred_dir, f"{image_id}_pred.nii.gz")
